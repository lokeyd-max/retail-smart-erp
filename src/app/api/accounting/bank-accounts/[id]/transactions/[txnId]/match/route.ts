...
import { matchBankTransactionSchema } from '@/lib/validation/schemas/accounting'
import { z } from 'zod'
...
export async function POST(
  ...
  try {
    ...
    const paramsParsed = validateParams(await params, z.object({ id: z.string().uuid(), txnId: z.string().uuid() }))
    if (!paramsParsed.success) return paramsParsed.response
    const { id, txnId } = paramsParsed.data
    const parsed = await validateBody(request, matchBankTransactionSchema)
    if (!parsed.success) return parsed.response
    const { voucherType, voucherId } = parsed.data
    ...
    return await withTenant(tenantId, async (db) => {
      ...
      // Fetch the transaction and verify it belongs to this bank account
      ...
      if (transaction.status === 'reconciled') {
        return NextResponse.json(
          { error: 'Cannot match an already reconciled transaction' },
          { status: 400 }
        )
      }
      ...
      const [updated] = await db
        ...
        .set({
          status: 'matched',
          matchedVoucherType: voucherType,
    ...
    logError('api/accounting/bank-accounts/[id]/transactions/[txnId]/match', error)

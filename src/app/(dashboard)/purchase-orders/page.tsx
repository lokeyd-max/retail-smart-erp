...
import {
  ...
  Home, ArrowUp, ArrowDown, ArrowUpDown,
  Send, CheckCircle, Ban, Loader2
} from 'lucide-react'
...
export default function PurchaseOrdersPage() {
  ...
  // Paginated purchase orders with server-side search
  const {
    data: orders,
    pagination,
    loading,
    ...
    setAdditionalParams,
    refresh,
  } = usePaginatedData<PurchaseOrder>({
    endpoint: '/api/purchase-orders',
    entityType: 'purchase-order',
    storageKey: 'purchase-orders-page-size',
  })
  ...
  function handleRowClick(orderId: string) {
    router.push(tenantSlug ? `/c/${tenantSlug}/purchase-orders/${orderId}` : `/purchase-orders/${orderId}`)
  }
  ...
  async function handleBulkAction(action: 'submit' | 'confirm' | 'cancel' | 'delete', cancellationReason?: string) {
    if (selectedIds.size === 0) return
    if (action === 'delete' && !confirm(`Delete ${selectedIds.size} purchase order(s)? This cannot be undone.`)) return
    setBulkProcessing(true)
    setShowActionsMenu(false)
    try {
      const res = await fetch('/api/purchase-orders/bulk-action', {
        method: 'POST',
      ...
      const label = action === 'submit' ? 'Submitted' : action === 'confirm' ? 'Confirmed' : action === 'cancel' ? 'Cancelled' : 'Deleted'
      toast.success(`${label} ${data.processed} of ${data.total} order(s)`)
      if (data.errors?.length > 0) {
        data.errors.forEach((err: string) => toast.error(err))
      }
      if (data.skipped?.length > 0) {
        data.skipped.forEach((msg: string) => toast.info(msg))
      }
      clearSelection()
      refresh()
    } catch {
  ...
  if (loading && orders.length === 0) {
    return <PageLoading text="Loading purchase orders..." />
  }
    ...
    <div className="h-full flex flex-col -m-5">
      ...
      <div className="px-4 py-2 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 text-sm">
        <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400">
          ...
          <ChevronRight size={14} />
          <span className="text-gray-900 dark:text-white font-medium">Purchase Order</span>
        </div>
      </div>
      ...
      <div className="px-4 py-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Purchase Order</h1>
          <button
            onClick={() => router.push(tenantSlug ? `/c/${tenantSlug}/purchase-orders/new` : '/purchase-orders/new')}
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded text-sm transition-colors disabled:opacity-50"
              >
                Actions
                <ChevronDown size={14} />
              </button>
              {showActionsMenu && (
                <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-gray-800 rounded shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-50">
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2 text-left text-sm text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20 flex items-center gap-2"
                  >
                    <Ban size={14} />
                    Cancel
                  </button>
          ...
          <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
              ...
              <button
                ...
                className="w-40 px-4 py-3 flex items-center gap-1 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-left"
              >
                Status
          ...
          <div className="flex-1 overflow-y-auto">
            {orders.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 px-4">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-1">No Purchase Orders</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                  {search || hasFilters ? 'No orders match your search criteria' : 'Get started by creating your first purchase order'}
                </p>
                {!search && !hasFilters && (
                  <button
                    onClick={() => router.push(tenantSlug ? `/c/${tenantSlug}/purchase-orders/new` : '/purchase-orders/new')}
                    ...
                    <div className="w-40 px-4 py-3">
                      <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${status.color}`}>
                        <span className={`w-2 h-2 rounded-full ${
                          ...
                          order.status === 'submitted' ? 'bg-orange-500' :
                          order.status === 'confirmed' ? 'bg-blue-500' :
                          order.status === 'invoice_created' ? 'bg-green-500' :
      ...
      <CancellationReasonModal
        ...
        onConfirm={(reason) => {
          ...
          handleBulkAction('cancel', reason)
        }}
        title="Cancel Purchase Orders"
        itemName={`${selectedIds.size} selected purchase order(s)`}

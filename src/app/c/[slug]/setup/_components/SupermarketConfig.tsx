...
import { supermarketCategories } from '@/lib/setup/seed-data'
...
export function SupermarketConfig({ data, onChange, suggestions, suggestionsLoading, dismissed, onDismiss }: SupermarketConfigProps) {
  ...
  useEffect(() => {
    if (data.selectedCategories.length === 0) {
      const defaults = supermarketCategories.filter(c => c.selected).map(c => c.name)
      onChange({ selectedCategories: defaults })
    }
  }, [data.selectedCategories.length, onChange])

  const allCategories = [
    ...supermarketCategories.map(c => c.name),
    ...data.selectedCategories.filter(c => !supermarketCategories.find(r => r.name === c)),
  ]
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
          Select the departments you want to start with. You can add more later.
        </p>
      ...
      {data.selectedCategories.filter(c => !supermarketCategories.find(r => r.name === c)).length > 0 && (
        <div className="flex flex-wrap gap-2 mt-2">
          {data.selectedCategories
            .filter(c => !supermarketCategories.find(r => r.name === c))
            .map(name => (

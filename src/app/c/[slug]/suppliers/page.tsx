...
import { Plus, Pencil, Trash2, Phone, Mail, Building2, X, History, ArrowRight, ExternalLink, Download, Upload, Package } from 'lucide-react'
...
export default function SuppliersPage() {
  ...
  // Item pricing state
  interface ItemCostRow { id: string; itemId: string; itemName: string; itemSku: string | null; lastCostPrice: string; totalPurchasedQty: string; lastPurchaseDate: string | null; lastPurchaseNo: string | null }
  const [itemCosts, setItemCosts] = useState<{ data: ItemCostRow[]; pagination: { page: number; pageSize: number; total: number; totalPages: number } } | null>(null)
  const [itemCostPage, setItemCostPage] = useState(1)
  const [loadingItemCosts, setLoadingItemCosts] = useState(false)
  ...
  // Fetch item costs for selected supplier
  const fetchItemCosts = useCallback(async () => {
    if (!selectedSupplier) return
    setLoadingItemCosts(true)
    try {
      const res = await fetch(`/api/suppliers/${selectedSupplier.id}/item-costs?page=${itemCostPage}&pageSize=10`)
      if (res.ok) setItemCosts(await res.json())
    } catch { /* ignore */ }
    setLoadingItemCosts(false)
  }, [selectedSupplier, itemCostPage])
  ...
  useRealtimeData(fetchItemCosts, {
    entityType: ['item-supplier-cost', 'purchase'],
    enabled: !!selectedSupplier,
    refreshOnMount: false,
  })
  ...
  }, [selectedSupplier, fetchBalanceHistory, fetchItemCosts])
  ...
  return (
    <PermissionGuard permission="managePurchases">
    <ListPageLayout
      module="Buying"
      moduleHref="/buying"
      title="Supplier"
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className={`font-medium ${parseFloat(supplier.balance) < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'}`}>
                            {formatCurrency(parseFloat(supplier.balance), currency)}
                          </span>
                        </td>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded p-4 border border-blue-100 dark:border-blue-800">
                ...
                <div className={`text-3xl font-bold ${parseFloat(balanceHistory?.supplier?.currentBalance || selectedSupplier.balance) < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'}`}>
                  {formatCurrency(parseFloat(balanceHistory?.supplier?.currentBalance || selectedSupplier.balance), currency)}
                </div>
                            ...
                            return (
                              <tr key={record.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td className={`px-3 py-2 text-right font-medium ${isIncrease ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                  {isIncrease ? '+' : ''}{formatCurrency(amount, currency)}
                                </td>
                                <td className="px-3 py-2">
                                  <div className="flex items-center justify-center gap-1 text-xs text-gray-500">
                                    <span>{formatCurrency(parseFloat(record.previousBalance), currency)}</span>
                                    <ArrowRight size={12} />
                                    <span className="font-medium text-gray-700 dark:text-gray-300">
                                      {formatCurrency(parseFloat(record.newBalance), currency)}
                                    </span>
                                  </div>
                                </td>
                                <td className="px-3 py-2">
              ...
              {/* Item Pricing */}
              <SectionCard title="Item Pricing" icon={<Package size={16} />} collapsible defaultCollapsed
                actions={itemCosts && <span className="text-sm text-gray-500">{itemCosts.pagination.total} items</span>}
              >
                {loadingItemCosts ? (
                  <div className="flex items-center justify-center py-8">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
                  </div>
                ) : !itemCosts || itemCosts.data.length === 0 ? (
                  <p className="text-sm text-gray-500 dark:text-gray-400 py-4 text-center">
                    No items purchased from this supplier yet
                  </p>
                ) : (
                  <>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-gray-50 dark:bg-gray-700">
                          <tr>
                            ...
                            <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Last Cost</th>
                            <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Total Qty</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Last Purchase</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                          {itemCosts.data.map((ic) => (
                            <tr key={ic.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                              <td className="px-3 py-2">
                                <div className="text-gray-900 dark:text-gray-100">{ic.itemName}</div>
                                {ic.itemSku && <div className="text-xs text-gray-400">{ic.itemSku}</div>}
                              </td>
                              <td className="px-3 py-2 text-right font-mono text-gray-900 dark:text-gray-100">
                                {formatCurrency(parseFloat(ic.lastCostPrice), currency)}
                              </td>
                              <td className="px-3 py-2 text-right text-gray-600 dark:text-gray-400">
                                {parseFloat(ic.totalPurchasedQty).toFixed(0)}
                              </td>
                              <td className="px-3 py-2 text-gray-600 dark:text-gray-400">
                                {ic.lastPurchaseDate ? new Date(ic.lastPurchaseDate).toLocaleDateString() : '-'}
                                {ic.lastPurchaseNo && <div className="text-xs text-gray-400">{ic.lastPurchaseNo}</div>}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    {itemCosts.pagination.totalPages > 1 && (
                      <div className="flex items-center justify-between pt-3 border-t dark:border-gray-700 mt-3">
                        <span className="text-sm text-gray-500">
                          Page {itemCosts.pagination.page} of {itemCosts.pagination.totalPages}
                        </span>
                        <div className="flex gap-2">
                          <Button variant="outline" size="sm" onClick={() => setItemCostPage(p => Math.max(1, p - 1))} disabled={itemCostPage === 1}>Previous</Button>
                          <Button variant="outline" size="sm" onClick={() => setItemCostPage(p => Math.min(itemCosts.pagination.totalPages, p + 1))} disabled={itemCostPage === itemCosts.pagination.totalPages}>Next</Button>
                        </div>
                      </div>
                    )}
                  </>

...
const exportColumns: ExportColumn[] = [
  { key: 'itemName', header: 'Menu Item', width: 25 },
  { key: 'categoryName', header: 'Category', width: 15 },
  { key: 'timesOrdered', header: 'Qty Ordered', format: 'number', width: 12 },
...
export default function MenuPerformancePage() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/menu-performance?${params}`)
  ...
  const totalOrdered = data?.reduce((s, d) => s + d.timesOrdered, 0) || 0
  ...
  return (
    <ReportPageLayout
      ...
      emptyIcon={<ChefHat size={40} />}
      emptyMessage="Select a date range to view menu performance."
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      exportData={data as any}
      exportColumns={exportColumns}
      exportName="Menu Performance"
      summaryCards={data ? <>
        <SummaryCard label="Total Revenue" value={formatCurrency(totalRevenue)} color="blue" />
        <SummaryCard label="Total Ordered" value={totalOrdered.toString()} color="green" />
        <SummaryCard label="Menu Items" value={(data?.length || 0).toString()} color="purple" />
      </> : undefined}
      chart={top10.length > 0 ? (
        <BarChart labels={top10.map((d) => (d.itemName || "Unknown").slice(0, 20))} datasets={[{ label: 'Revenue', data: top10.map((d) => d.totalRevenue) }]} height={250} color="emerald" />
      ) : undefined}
    >
      {data && (
          ...
          <div className="overflow-x-auto list-container-xl">
            <table className="w-full">
              <thead className="table-sticky-header bg-gray-50 dark:bg-gray-900">
                <tr>
                  <th className="px-4 py-2 text-left text-sm font-medium text-gray-600 dark:text-gray-400">Menu Item</th>
                  <th className="px-4 py-2 text-left text-sm font-medium text-gray-600 dark:text-gray-400">Category</th>
                  <th className="px-4 py-2 text-right text-sm font-medium text-gray-600 dark:text-gray-400">Qty Ordered</th>
              ...
              <tbody>
                {data.map((row, i) => (
                  <tr key={i} className="border-t border-gray-100 dark:border-gray-700">
                    ...
                    <td className="px-4 py-2.5 text-sm text-right text-gray-700 dark:text-gray-300">{row.timesOrdered}</td>
                    <td className="px-4 py-2.5 text-sm text-right text-gray-600 dark:text-gray-400">{row.orderCount}</td>

...
interface PurchaseSummaryData {
  summary: { totalPurchases: number; totalReturns: number; netPurchases: number; orderCount: number }
  groupLabel: string
  data: { group: string; totalPurchases: number; totalReturns: number; netPurchases: number; orderCount: number }[]
}
...
const exportColumns: ExportColumn[] = [
  ...
  { key: 'totalPurchases', header: 'Total Purchases', format: 'currency', width: 15 },
  { key: 'totalReturns', header: 'Returns', format: 'currency', width: 15 },
  { key: 'netPurchases', header: 'Net Purchases', format: 'currency', width: 15 },
...
export default function PurchaseSummaryPage() {
  ...
  const [data, setData] = useState<PurchaseSummaryData | null>(null)
  ...
  return (
    <ReportPageLayout
      ...
      emptyMessage="Select a date range to view purchase summary."
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      exportData={data?.data as any}
      exportColumns={exportColumns}
      exportName="Purchase Summary"
      summaryCards={data ? <>
        <SummaryCard label="Total Purchases" value={formatCurrency(data.summary.totalPurchases)} color="blue" />
        <SummaryCard label="Returns" value={formatCurrency(data.summary.totalReturns)} color="red" />
        <SummaryCard label="Net Purchases" value={formatCurrency(data.summary.netPurchases)} color="green" />
        <SummaryCard label="Orders" value={data.summary.orderCount.toString()} color="purple" />
      </> : undefined}
      chart={data && data.data.length > 0 ? (
        <BarChart labels={data.data.map((d) => d.group)} datasets={[{ label: 'Net Purchases', data: data.data.map((d) => d.netPurchases) }]} height={250} color="emerald" />
      ) : undefined}
    >
      {data && (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-900">
                <tr>
                  ...
                  <th className="px-4 py-2 text-right text-sm font-medium text-gray-600 dark:text-gray-400">Net</th>
              ...
              <tbody>
                {data.data.map((row, i) => (
                  <tr key={i} className="border-t border-gray-100 dark:border-gray-700">
                    ...
                    <td className="px-4 py-2.5 text-sm text-right font-mono tabular-nums">{formatCurrency(row.totalPurchases)}</td>
                    ...
                    <td className="px-4 py-2.5 text-sm text-right font-mono tabular-nums font-semibold">{formatCurrency(row.netPurchases)}</td>
                    <td className="px-4 py-2.5 text-sm text-right">{row.orderCount}</td>
                  </tr>
                ))}
              </tbody>
            </table>

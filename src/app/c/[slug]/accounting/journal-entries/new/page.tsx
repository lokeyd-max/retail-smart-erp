...
import { Home, ChevronRight, Plus, X, Loader2, Save, Send } from 'lucide-react'
...
export default function NewJournalEntryPage() {
  ...
  function validate(): boolean {
    ...
    // Fix #15: Match server-side validation - require exact balance (rounded to 2dp)
  ...
  async function handleSave(submitAfterSave: boolean) {
    ...
    try {
      ...
      const entryId = created.id || created.data?.id

      if (submitAfterSave && entryId) {
        const submitRes = await fetch(`/api/accounting/journal-entries/${entryId}/submit`, {
          method: 'POST',
        })
        if (!submitRes.ok) {
          const submitData = await submitRes.json()
          toast.error(submitData.error || 'Entry saved but failed to submit')
          router.push(`/c/${tenantSlug}/accounting/journal-entries/${entryId}`)
          return
        }
        toast.success('Journal entry created and submitted')
      } else {
  ...
  return (
    <div className="space-y-4 p-4 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-gray-900 dark:text-white">New Journal Entry</h1>
      </div>
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        <button
          ...
          disabled={saving}
          className="flex items-center gap-1.5 px-4 py-2 text-sm border border-blue-600 text-blue-600 dark:text-blue-400 dark:border-blue-400 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >

...
export default function InspectionPage() {
  ...
  async function handleDeletePhoto(photoId: string) {
    ...
    try {
      const res = await fetch(`/api/work-orders/${workOrderId}/inspections/${inspection.id}/photos?photoId=${photoId}`, {
        method: 'DELETE',
      })
      if (res.ok) {
        await fetchPhotos(inspection.id)
  ...
  async function handleComplete() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/c/${tenantSlug}/work-orders/${workOrderId}`)
      } else {
        const data = await res.json()
  ...
  if (!workOrder || !inspection) {
    return (
      <div className="text-center py-12">
        ...
        <Link href={`/c/${tenantSlug}/work-orders/${workOrderId}`} className="text-blue-600 hover:underline mt-2 inline-block">
          Back to Work Order
        </Link>
      </div>
    )
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Link href={`/c/${tenantSlug}/work-orders/${workOrderId}`} className="p-2 hover:bg-gray-100 rounded">
          <ChevronLeft size={20} />
        </Link>

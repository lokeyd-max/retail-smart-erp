...
import { Plus, Pencil, Trash2, Car, ShoppingCart, Wrench, FileText, X, ExternalLink, Download, Upload } from 'lucide-react'
...
export default function CustomersPage() {
  ...
  return (
    ...
    >
      ...
      <div className={`${selectedCustomer ? 'w-1/2' : 'w-full'} flex flex-col transition-all duration-300`}>
            ...
            <table className="w-full">
              ...
              <thead className="bg-gray-50 dark:bg-gray-700 table-sticky-header">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">City</th>
                ...
                {customers.length === 0 ? (
                  ...
                  customers.map((customer) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{customer.city || '-'}</td>
              ...
              <>
                ...
                <SectionCard title="Contact Information" icon={<Car size={16} />} collapsible defaultCollapsed>
                  <FieldGrid columns={2}>
                    ...
                    <Field label="Credit Limit" value={selectedCustomer.creditLimit ? formatCurrency(parseFloat(selectedCustomer.creditLimit), currency) : null} />
                  </FieldGrid>
                </SectionCard>
                    ...
                    <div className="space-y-2">
                      {sales.slice(0, 10).map((sale) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3">
                            <span className="font-medium text-gray-900 dark:text-white">{formatCurrency(parseFloat(sale.total), currency)}</span>
                    ...
                    <div className="space-y-2">
                      {workOrders.slice(0, 10).map((wo) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3">
                            <span className="font-medium text-gray-900 dark:text-white">{formatCurrency(parseFloat(wo.grandTotal || '0'), currency)}</span>
                            <span className="text-xs text-gray-500">
                              {fDate(wo.createdAt)}
                            </span>
                            <ExternalLink size={14} className="text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                    ...
                    <div className="space-y-2">
                      {estimates.slice(0, 10).map((est) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3">
                            <span className="font-medium text-gray-900 dark:text-white">{formatCurrency(parseFloat(est.grandTotal || '0'), currency)}</span>
                            <span className="text-xs text-gray-500">
                              {fDate(est.createdAt)}
                            </span>
                            <ExternalLink size={14} className="text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity" />
